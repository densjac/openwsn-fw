...
// Requirements:
// To trigger: 
// Verify that there has been a change in the number of used
// against any fo the neighbours.
// For each neighbour, if it has changed the number of used cells from
// the previous slotframe:
// Calculate the difference in the number of cells between timeslots: NEWCELL
// - Estimate the number of cells: NEWCELL+OVERPROVISION Even if NEWCELL is 
// negative
...
// - If new cells must be allocated, select a random number of cells from the available ones on the
